...
There are `r ncol(Whickham)` variables in this dataset. Both `outcome` and `smoker` are character data that are 2 level factors, and `age` is numerical data.
...
```{r}
library(ggplot2)
ggplot(Whickham, aes(x = age, fill = age)) +
...
I expect smokers are more likely to die.
...
Whickham %>%
  ...
  group_by(smoker) %>%
  mutate(prop.ddead = n/sum(n))
```
This does not meet my expectations. It would appear with this data, that people who smoke have a higher chance of living longer.
